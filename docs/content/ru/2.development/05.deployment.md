---
title: Деплой
description: Деплой EliteMoney и документации на Vercel из монорепы с Supabase и PowerSync в качестве бэкенда.
navigation:
  icon: i-lucide-rocket
seo:
  title: Деплой
  description: Как задеплоить приложение и документацию EliteMoney на Vercel с Supabase Postgres и PowerSync.
---

EliteMoney - это монорепа на pnpm workspaces. Приложение и документация деплоятся из одного репозитория как два отдельных проекта в Vercel.
...
1. Создайте проект Supabase (через дашборд или `supabase projects create`), привяжите репозиторий и примените схему ([доки Supabase по миграциям](https://supabase.com/docs/guides/deployment/database-migrations)):
   ...
   ```bash [Terminal]
   cd app
   supabase projects create elitemoney --org-id <org-id> --region <region> --db-password <pw>
...
3. В **Authentication -> URL Configuration** задайте **Site URL** = origin вашего приложения (например, `https://elitemoney.vercel.app`) и добавьте его (с шаблоном `/**`) в **Redirect URLs**, чтобы возврат после OAuth на `/login` был разрешён.
...
## Premium (EliteMoney Premium)

EliteMoney Premium расширяет базовое приложение как **Nuxt layer**.
...
- `base/` - Git-субмодуль, указывающий на ветку `main` репозитория `awejofficial/elitemoney`
...
Субмодуль должен указывать на ветку `main` репозитория `awejofficial/elitemoney`:

```bash [Terminal]
git submodule add -b main https://github.com/awejofficial/elitemoney.git base
```
