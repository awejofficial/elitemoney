---
title: Deployment
description: Deploy EliteMoney and docs to Vercel from a monorepo, with Supabase and PowerSync as the backend.
navigation:
  icon: i-lucide-rocket
seo:
  title: Deployment
  description: How to deploy EliteMoney app and docs to Vercel with Supabase Postgres and PowerSync.
---

EliteMoney is a pnpm workspace monorepo. Both the app and docs deploy from the same repository as separate Vercel projects.
...
3. In **Authentication -> URL Configuration**, set **Site URL** to your deployed app origin (e.g. `https://elitemoney.vercel.app`) and add it (with a `/**` wildcard) to **Redirect URLs**, so the OAuth return to `/login` is allowed.
...
EliteMoney Premium extends the base app as a **Nuxt layer**.
...
- `base/` is a Git submodule pointing to the `main` branch of `awejofficial/elitemoney`
...
The submodule should point to the `main` branch of `awejofficial/elitemoney`:
