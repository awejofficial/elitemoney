...
seo:
  title: Offline & PWA
  description: How EliteMoney works offline as a Progressive Web App, backed by PowerSync's local-first sync.
---

## PWA Installation

EliteMoney is a Progressive Web App (PWA). You can install it on your device for a native app-like experience:
...
EliteMoney is offline-first. All your data lives in a local SQLite database in the browser (backed by PowerSync, persisted in IndexedDB). You can create, edit, and delete transactions, wallets, and categories while offline - reads and writes always hit the local database first, so the UI never waits on the network.
