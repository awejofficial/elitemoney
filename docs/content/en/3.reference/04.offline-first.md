...
seo:
  title: Offline-first
  description: EliteMoney is offline-first via PowerSync - local SQLite holds all data, writes are optimistic and queued locally, fatal upload errors are discarded and surfaced via toast.
---

EliteMoney is offline-first via PowerSync. Local SQLite is the single source of truth. All reads come from SQLite; all writes go to SQLite first and are queued for upload to Supabase Postgres.
